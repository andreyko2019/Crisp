...
export class FeaturedSwiper {
  ...
  renderSlides() {
    ...
    this.slidesArr.forEach((item) => {
      const slide = renderElement('div', ['featured-items__swiper-slide', 'swiper-slide']);

      const cardLink = renderElement('a', ['card', 'featured-items__card', item.id]) as HTMLAnchorElement;
